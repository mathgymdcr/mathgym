...
- `cubo-transportista` → un cubo rueda por una rejilla NxN (`rollCube` en `scripts/cubo-logic.js`) y hay que llevarlo a la meta mostrando una cara concreta arriba, en el mínimo de movimientos (BFS). El payload es `{ tablero: {ancho, alto}, inicio: {f, c, orientacion}, meta: {f, c, cara}, minimo }`, donde `orientacion` es `{U, D, N, S, E, O}` (qué cara 1-6 mira hacia cada lado). No hay `winCondition: grid_matches_solution` aquí: se puntúa por movimientos (`parMoves`), como anillas-encadenadas y cajas-apiladas.
- `desenreda-la-red` → el único tipo sin rejilla: un árbol de nodos y cables (`{ nodos, aristas: [[a,b],...], posiciones_iniciales: [{x,y},...] }`, posiciones en porcentaje) que hay que desenredar arrastrando nodos hasta que ningún cable cruce a otro. Sin `solucion`: cualquier disposición final sin cruces vale, no hay una única respuesta. `cuentaCruces`/`segmentosSeCruzan` en `scripts/red-logic.js` son la fuente de verdad de qué cuenta como cruce, compartida por plantilla, generador y validador.
- `trazo-perimetral` → Slitherlink: trazar un único circuito cerrado sobre los puntos de una rejilla NxN de celdas, cumpliendo en cada celda numerada cuántos de sus 4 lados forman parte del circuito. El payload es `{ tablero: {ancho, alto}, pistas: [{f, c, valor}], solucion }`, donde `solucion` es una rejilla de booleanos (dentro/fuera del circuito) -- **no** la lista de aristas. `scripts/trazo-logic.js` reformula el puzzle como 2-coloreado de celdas (la técnica estándar de solvers de Slitherlink reales) en vez de backtracking sobre aristas: evita comprobar "es un solo bucle" como restricción global tardía, que es lo que hace lento a Slitherlink en general.
