...
// `Math.floor(seed / 12)` (el seed en crudo) o con los días reales
// directamente: ambas fallaban porque el hueco real entre dos ocurrencias de
// nonograma NO es constante -- depende de qué otros tipos compiten por
// selectTemplate y de los saltos del seed al cruzar un cambio de mes -- así
// que un hueco que por coincidencia fuera múltiplo del tamaño del banco
// (3-6 figuras) repetía dibujo sí o sí. Contar ocurrencias de verdad no
// tiene ese problema: el conteo SIEMPRE avanza exactamente 1 de una
// ocurrencia a la siguiente, sea cual sea el hueco.
...
export function buildNonogramaPuzzle(seed) {
  ...
  // Rota el banco con cuántas veces le tocó nonograma antes de esta fecha
  // (ver ocurrenciasPrevias), no con el seed en crudo: eso avanza en +1
  // exacto por ocurrencia, sea cual sea el hueco real en días, así que no
  // repite dibujo hasta que el banco entero (3-6 figuras) dé la vuelta.
