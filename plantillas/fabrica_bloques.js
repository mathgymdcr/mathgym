...
// coartada afirma un indicio en una celda concreta. Dos dicen la verdad
// (su afirmación coincide con `solucion`), uno miente -- ahí está el
// culpable. Esas celdas, quién miente y con qué valor falso los sortea
// `generaSospechosos` en fabrica-logic.js a partir del SEED, no del reto ya
// construido, así que la prueba sale gratis de una solución que ya es
// única. Los 3 nombres por caso (`CASOS[caso].sospechosos`) son cosa de
// esta plantilla, igual que el resto del texto narrativo.
...
export const CASOS = {
  'joyeria-del-mar': {
    ...
    sospechosos: [{ nombre: 'El vigilante nocturno' }, { nombre: 'La dependienta de la tarde' }, { nombre: 'El técnico de la alarma' }]
  },
  'cargamento-fantasma': {
    ...
    sospechosos: [{ nombre: 'El jefe de turno de tarde' }, { nombre: 'El conductor de la furgoneta' }, { nombre: 'La encargada de inventario' }]
  },
  'correo-interceptado': {
    ...
    sospechosos: [{ nombre: 'La becaria de prácticas' }, { nombre: 'El jefe de ventas' }, { nombre: 'El consultor externo' }]
  },
  'examen-filtrado': {
    ...
    sospechosos: [{ nombre: 'El bedel' }, { nombre: 'La profesora de guardia' }, { nombre: 'El administrador del servidor' }]
  },
  'amano-deportivo': {
    ...
    sospechosos: [{ nombre: 'El árbitro' }, { nombre: 'El capitán del equipo' }, { nombre: 'El delegado de campo' }]
  },
  'etiquetado-fraudulento': {
    ...
    sospechosos: [{ nombre: 'El chef' }, { nombre: 'El proveedor habitual' }, { nombre: 'El jefe de sala' }]
  },
  'obra-con-material-sustituido': {
    ...
    sospechosos: [{ nombre: 'El jefe de obra' }, { nombre: 'El proveedor de cemento' }, { nombre: 'El aparejador' }]
  },
  'taller-de-piezas-falsificadas': {
    ...
    sospechosos: [{ nombre: 'El dueño del taller' }, { nombre: 'El mecánico de turno' }, { nombre: 'El proveedor de recambios' }]
  },
  'laboratorio-manipulado': {
    ...
    sospechosos: [{ nombre: 'El técnico de laboratorio' }, { nombre: 'La directora del laboratorio' }, { nombre: 'El comercial de certificaciones' }]
  },
  'festival-de-entradas-falsas': {
    ...
    sospechosos: [{ nombre: 'El jefe de taquilla' }, { nombre: 'La responsable de sistemas' }, { nombre: 'El encargado de puertas' }]
  },
  'distribuidora-farmaceutica': {
    ...
    sospechosos: [{ nombre: 'El transportista' }, { nombre: 'El farmacéutico asociado' }, { nombre: 'El encargado de almacén' }]
  },
  'museo-de-la-pieza-falsa': {
    ...
    sospechosos: [{ nombre: 'El restaurador' }, { nombre: 'El comisario de la exposición' }, { nombre: 'El tasador del seguro' }]
  }
};
...
export async function render(root, data, hooks) {
  ...
  const ui = buildStandardShell({
    ...
    instructionsHTML: `
      ...
      <ul>
        ...
        <li>Con la rejilla ya rellena, compara la coartada de cada sospechoso con lo que de verdad anotaste en su franja y zona, y señala a quien mintió.</li>
      </ul>
    `
  });
  ...
  // Cabeceras F1..Fn / Z1..Zn: hacen falta para que el jugador pueda ubicar
  // la celda que menciona la coartada de cada sospechoso (fila+columna),
  // que hasta ahora no se leían en ningún sitio del tablero.
  const tablero = createElement('div', { class: 'fabrica-tablero' });
  tablero.style.gridTemplateColumns = `auto repeat(${n}, 1fr)`;
  ...
  // franja/zona -- la prueba es el tablero ya resuelto, no un dato aparte.
  ...
  sospechosos.forEach((s, i) => {
    ...
    const boton = createElement('button', { class: 'fabrica-sospechoso', type: 'button' });
    const nombreSpan = createElement('span', { class: 'fabrica-sospechoso-nombre' });
    nombreSpan.textContent = perfil.nombre;
    const coartadaSpan = createElement('span', { class: 'fabrica-sospechoso-coartada' });
    coartadaSpan.textContent = `«Yo anoté el indicio nº${s.valorAfirmado}, zona ${s.columna + 1}, franja ${s.fila + 1}.»`;
    boton.appendChild(nombreSpan);
    boton.appendChild(coartadaSpan);
    boton.addEventListener('click', () => {
      ...
      if (i === culpable) {
        ...
        panelSospechosos.querySelectorAll('.fabrica-sospechoso').forEach((b) => { b.disabled = true; });
        boton.classList.add('culpable');
      } else {
        fallosAcusacion++;
        boton.disabled = true;
        boton.classList.add('descartado');
        setStatus(ui.result, 'Esa coartada cuadra con lo que anotaste. No es tu sospechoso.', 'ko');
      }
    });
    panelSospechosos.appendChild(boton);
  });
